...
class BaseNutritionAPI(ABC):
    ...
    def __init__(
        self, api_key: Optional[str] = None, rate_limit: Optional[tuple] = None
    ):
...
class USDAFoodDataAPI(BaseNutritionAPI):
    ...
    def _parse_usda_food(self, food_data: Dict) -> NutritionData:
        ...
        if portions:
            portion = portions[0]
            serving_size = (
                f"{portion.get('amount', '')} {portion.get('modifier', '')}".strip()
            )
        ...
        calories = nutrients.get("energy") or nutrients.get(
...
class NutritionAPIManager:
    ...
    def search_foods(
        self, query: str, providers: Optional[List[APIProvider]] = None, limit: int = 10
    ) -> Dict[str, List[NutritionData]]:
    ...
    def get_food_details(
        self, provider: APIProvider, food_id: str
...
if __name__ == "__main__":
    ...
    if results and APIProvider.USDA_FOODDATA.value in results:
        ...
        details = manager.get_food_details(
            APIProvider.USDA_FOODDATA, first_food.food_id
        )
        if details:
